...
- **Preview/staging**: a Vercel cria automaticamente uma URL de preview para cada Pull
  ...
  **segundo serviço no Render** (ex: `myplanner-api-staging`) apontando para uma
...
2. **New → Blueprint** → selecione o repositório `myplanner` → Render detecta o
   `render.yaml` automaticamente e propõe criar o banco `myplanner-db` e o serviço
   web `myplanner-api`.
...
4. Depois do primeiro deploy, anote a URL pública da API (algo como
   `https://myplanner-api.onrender.com`).
...
2. **Add New → Project** → selecione `myplanner`.
3. Em **Root Directory**, selecione `apps/web` (importante — o `vercel.json` de
   dentro dessa pasta assume isso).
4. Em **Environment Variables**, adicione:
   - `VITE_API_URL` = `https://myplanner-api.onrender.com/api` (a URL do Render do
     passo anterior, com `/api` no final)
5. Deploy. Anote a URL pública (ex: `https://myplanner.vercel.app`).

### Fechando o ciclo: CORS

Volte no Render, no serviço da API, e defina:
- `CORS_ORIGIN` = `https://myplanner.vercel.app` (a URL da Vercel do passo anterior)
